...
class ModelOutputs():
    ...
    def __call__(self, x):
        ...
        # target_activations_array = np.array(target_activations)

        # print("target_activations:",target_activations_array.shape)
...
class GradCam:
    ...
    def __call__(self, input, index=None):
        ...
        # cam = cv2.resize(cam, (412, 585))
        cam = cv2.resize(cam, (414, 585))
        cam = cam - np.min(cam)
        cam = cam / np.max(cam)
        return cam
